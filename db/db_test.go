...
func TestMutipleSearchCallbacks(t *testing.T) {
	ctx := context.Background()
	...
	app := st.Appender(ctx)
	for i := 0; i != 10_000; i++ {
		app.Append(0, labels.FromStrings("__name__", "foo", "bar", fmt.Sprintf("%d", 2*i)), 0, float64(i))
		app.Append(0, labels.FromStrings("__name__", "foo", "bar", fmt.Sprintf("%d", 2*i+1)), 0, float64(i))
	}
	...
	q1, err := engine.NewInstantQuery(ctx, db.Queryable(), nil, expr, time.Unix(5, 0))
	if err != nil {
	...
	for i := range v {
		if !strings.HasSuffix(v[i].Metric.Get("bar"), "1") {
			t.Fatalf("unexpected series in result for query %s: %s", expr, v[i].Metric)
		}
	}
}

func storageToDB(tb testing.TB, st *teststorage.TestStorage) *DB {
	ctx := context.Background()
	bkt := objstore.NewInMemBucket()
	...
	blkname, err := schema.BlockNameForDay(time.Unix(st.DB.Head().Meta().MinTime, 0).UTC())
	if err != nil {
		tb.Fatal(err)
	}
	meta, err := readMetafile(ctx, bkt, blkname)
	if err != nil {
		tb.Fatal(err)
	}
	blk, err := newBlockForMeta(ctx, bkt, meta)
	if err != nil {
		tb.Fatal(err)
	}
	return NewDB(singleBlockSyncer{blk: blk})
}

type singleBlockSyncer struct {
	blk *Block
}

func (s singleBlockSyncer) Blocks() []*Block {
	return []*Block{s.blk}
}
...
func BenchmarkQuery(b *testing.B) {
	...
	for i := 0; i != 100_000; i++ {
		for j := 0; j != 120; j++ {
			app.Append(0, labels.FromStrings("__name__", "foo", "bar", "fizz", "buzz", fmt.Sprintf("%d", i)), int64(j), float64(j))
		}
	}
	if err := app.Commit(); err != nil {
		b.Fatalf("unable to commit: %s", err)
	}
	db := storageToDB(b, st)

	qryable := db.Queryable()
	q1, err := engine.NewInstantQuery(ctx, qryable, nil, "foo", time.Unix(5, 0))
	if err != nil {
		b.Fatalf("error for query: %v", err)
	}
	defer q1.Close()
