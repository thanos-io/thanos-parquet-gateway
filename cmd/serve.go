...
func (opts *bucketOpts) registerServeFlags(cmd *kingpin.CmdClause) {
	cmd.Flag("storage.type", "type of storage").Default("filesystem").EnumVar(&opts.storage, "filesystem", "s3", "gcs")
	...
	cmd.Flag("storage.gcs.bucket", "GCS bucket").Default("").Envar("STORAGE_GCS_BUCKET").StringVar(&opts.gcsBucket)
	cmd.Flag("storage.gcs.service_account", "Inline GCS service account JSON").Default("").Envar("STORAGE_GCS_SERVICE_ACCOUNT").StringVar(&opts.gcsServiceAccount)
}
